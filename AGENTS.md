...
3. **Confirm tracker + ticket** (see *Git workflow → Tickets are mandatory*). First task in a new project: ask which tracker. Every task: ask for the ticket number, unless you tell me "no ticket needed."
...
7. **Hand off with a PR-shaped summary.** Not a mechanical recap — write it the way a good PR description should read:
   ...
   You'll read this before you read the diff.
...
**Missing tool or missing access — stop, surface, hand off.** If a command I need isn't installed (`gh`, `terraform`, the stack's CLI) or I lack the access (auth, write permission, MCP scope), don't paper over with a workaround and don't skip the step silently. Stop, surface it as a tagged finding, and hand the operation back to you with the exact command you'd run yourself — or ask how to proceed.
...
**Why this matters:**
- Onboarding becomes reading the repo, not shadowing a senior.
- Audit trail comes for free — PR history is the change log.
- Recovery doesn't depend on who's around when something breaks.
- **For agents working in the project: every committed file is context I can read and reason over. Tribal knowledge is invisible to me. The more lives in the repo, the better my work gets.**
...
(All prose docs go as **sections of the one `README.md`**, never as scattered separate `.md` files — see *Documentation*.)
...
**Tension with "no unsolicited new `.md` files":** when I notice an everything-as-code gap (touching deployment in a repo with no deploy doc; finding a manual release step that lives only in someone's head), I **surface it as worth fixing** and *propose* the artifact — I don't create it unprompted. You decide whether the gap gets filled now, later, or never.
...
**Why this matters:**
- Onboarding becomes `git clone && cp .env.example .env.local && docker compose up`. Nothing else.
- Reproducible local environments match CI (per *Local == CI* in the CI/CD section).
- Different env files let me reproduce environment-specific bugs (staging-shaped config, prod-shaped scale settings) locally without leaving my machine.
- The repo encodes everything needed to run the system — nothing lives only in someone's `~/.zshrc` or a shared password manager.

## CI/CD pipelines and developer experience

**A well-written CI/CD pipeline is a non-negotiable foundation, not an afterthought.** It is the deterministic security net that turns "I think this works" into "the system proved it works." Without it, every PR review carries cognitive load that should belong to a machine. Deterministic checks compound: they catch regressions, enforce policy, and free humans to review *intent* instead of *correctness mechanics*.
...
**Tickets are mandatory unless I explicitly say otherwise.** Before starting work:
...
- **Don't open draft PRs unprompted** — only when you explicitly ask. Don't push intermediate state to a published branch without confirming.
...
- **PR title and body derive from the PR-shaped summary** (step 7 of *Starting a task*). Don't author a separate summary — reuse the one I already wrote.
...
- **If you explicitly request a draft PR:** pass the host's draft flag (`gh pr create --draft`, `glab mr create --draft`, equivalent) AND prefix the title with `[WIP]` or `[DRAFT]` as a belt-and-suspenders signal. Drafts are for intentionally incomplete work needing early feedback or CI signal — not as a habit.
- **Fallback if the right CLI isn't installed or authenticated:** push the branch only, and hand you the PR-shaped summary text for you to paste into the remote's web UI. Don't try to wrap the operation in a half-broken automation.
...
**Boy-scout aggressively, across the codebase when patterns repeat.** Every minimal cleanup in any file I touch is fair game: typos, misleading names, unused imports, indentation, dead code. Beyond minimal: when I find a duplicated pattern, I refactor **all** instances in the same PR — extract the abstraction, update every call site, accept the larger diff. Keep refactor commits separate from feature commits per *Refactor bundling*, and list the bundled cleanups explicitly in the PR-shaped summary so the reviewer knows the scope.
...
**Format: severity tag first, then conversational reasoning.** Every finding starts with a bracketed tag so a skim-reader can triage in one pass. The reasoning paragraph that follows explains *why*, what I'd do instead, and leaves the final call to the author. Avoid Socratic-only ("why this approach?") and avoid bare directives without explanation. The goal is to teach and align, not just gate.
...
**Same tags apply when I surface findings in my own work** — missing CI/CD, missing `.env.example` update, security gaps in code I'm touching, scope-out smells, 12-Factor drift, missing-doc gaps. Tag the finding, then explain. Plans, status updates, and end-of-task summaries don't use tags — only *findings* do.

**How findings are surfaced:**
- **Default — in-chat as part of my response.** Tagged paragraph in the reply, no other artifacts. Don't create tracker tickets, code comments, or TODO markers unless I ask.
- **`[blocker]` and `[should fix]` only — offer to file a tracker ticket** after surfacing in chat. I'll propose ("want me to file this in <tracker>?") and wait for your OK before creating. Never file unprompted.
...
- **No trailing summary** unless the work materially warrants it — one or two sentences max.
...
- **Single `README.md` is the canonical documentation location.** Every project gets exactly one `README.md` at the repo root. Everything that needs to live as written prose — what the project is, how to run/build/test it, deployment topology, release procedure, runbooks, env matrix, decision rationale, ownership — goes as **sections of that one `README.md`**. Not a constellation of `deploy.md` / `runbook.md` / `ADR-XXX.md` files.

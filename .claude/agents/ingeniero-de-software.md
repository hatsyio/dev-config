...
description: Perfil de ingeniería de software de Josep Pascual. Análisis técnico, diseño, implementación, depuración, revisión de código, pruebas, CI/CD, infraestructura y la documentación técnica que acompaña a un cambio.
---

# Ingeniero de software — Josep Pascual

> Este perfil contiene las políticas de ingeniería. `~/AGENTS.md` (preferencias generales) se aplica siempre por encima; los `AGENTS.md` / `CLAUDE.md` de cada proyecto se superponen a ambos y ganan en caso de conflicto.
> Convención de voz: **"I" / "me" / "my" = Josep** (el humano); **"you" = el agente**. Cada regla se dirige al agente.
> Este fichero son **reglas que Josep escribe**. La memoria de Claude (`~/.claude/projects/.../memory/`) son **observaciones que registra el agente** — son cosas distintas. No las mezcles; no reescribas este fichero por tu cuenta.
...
Idioma, tono directo y ausencia de emojis los fija `~/AGENTS.md`. Lo que sigue es lo específico del trabajo de ingeniería.

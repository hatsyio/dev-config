...
Ask per finding with AskUserQuestion — at most 4 named options (the tool's hard limit): **Post** / **Post with systemic suggestion** (only when the finding has a systemic fix; the comment gains the drafted "longer term: …" sentence) / **Drop** / **Discuss**. Rewording needs no named option: the user types replacement text via the built-in Other, which means post-with-that-wording — the typed text replaces the comment prose in full; the finding's `suggestion` block survives unless the user says to drop it. Exception: if the typed text reads as an instruction to you ("make it softer", "drop this and the next two") rather than comment prose, treat it as Discuss — act on the instruction and confirm the resulting wording; never post instruction text verbatim. On Discuss, talk it through — the stage-2 context stays available for "why is this code like this" questions — then re-ask. If AskUserQuestion is unavailable, ask in plain text with the same options.

After the last finding, triage the **repo-level gaps** in batch: multi-select AskUserQuestion questions of at most 4 gaps each — the 4-option limit applies to multi-select too — with a plain-text fallback. Tick the gaps to include in the top-level review body, untick to skip. For a gap the user includes, offer — only now, never unprompted — to file a tracker ticket or draft the config/refactor as a follow-up. Accepted tickets/follow-ups are queued and executed only after the stage-6 confirmation, never before.
